...
class _AdminAuditLogScreenState extends ConsumerState<AdminAuditLogScreen> {
  ...
  Widget build(BuildContext context) {
    final entriesAsync = ref.watch(auditLogEntriesProvider(tableName: _tableName, from: _from, to: _to));
        ...
        children: [
          ...
          Expanded(
            child: entriesAsync.when(
              ...
              data: (entries) {
                if (entries.isEmpty) {
                  return const EmptyView(message: 'لا توجد عمليات مسجَّلة', icon: Icons.fact_check_outlined);
                }
...
class _AuditTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ExpansionTile(
      ...
      subtitle: Text('${entry.actorName ?? 'النظام'} · ${Formatters.dateTime(entry.createdAt)}'),
      children: [
        if (entry.oldData != null)
          Padding(
            ...
            child: Align(
              ...
              child: Text('قبل: ${entry.oldData}', style: Theme.of(context).textTheme.bodySmall),
            ),
          ),
        if (entry.newData != null)
          Padding(
            ...
            child: Align(
              alignment: Alignment.centerRight,
              child: Text('بعد: ${entry.newData}', style: Theme.of(context).textTheme.bodySmall),
            ),
          ),
