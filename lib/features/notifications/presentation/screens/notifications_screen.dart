    ...
    switch (n.type) {
      case 'order_status':
        ...
        if (role == AppRole.admin) {
          context.push(Routes.adminOrderDetail(orderId));
        }
      case 'maintenance_new':
      case 'maintenance_completed':
        if (maintenanceId == null) return;
        if (role == AppRole.customer) {
          context.push(Routes.customerMaintenanceDetail(maintenanceId));
        }
        if (role == AppRole.technician) {
          context.push(Routes.technicianMaintenanceDetail(maintenanceId));
        }
        if (role == AppRole.admin) {
          context.push(Routes.adminMaintenanceDetail(maintenanceId));
        }
      case 'low_stock':
