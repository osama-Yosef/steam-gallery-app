...
class MaintenanceDetailScreen extends ConsumerWidget {
  ...
  Future<void> _addImage(BuildContext context, WidgetRef ref, String customerId) async {
    ...
    try {
      await ref.read(maintenanceRepositoryProvider).uploadImage(requestId, customerId, bytes, ext);
  ...
  Future<void> _cancel(BuildContext context, WidgetRef ref) async {
    ...
    if (!confirmed) return;
    try {
      await ref.read(maintenanceRepositoryProvider).cancel(requestId, 'ألغاه العميل');
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(AppException.from(e).messageAr)));
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text('رقم الدور #${req.ticketNumber}', style: Theme.of(context).textTheme.titleLarge),
                          _StatusChip(status: req.status),
                        ],
                      ),
                      const SizedBox(height: 4),
                      Text(Formatters.dateTime(req.createdAt), style: Theme.of(context).textTheme.bodySmall),
                    ],
                  ),
                ),
              ),
              if (isActive) ...[
                ...
                Consumer(
                  builder: (context, ref, _) {
                    final posAsync = ref.watch(myQueuePositionProvider(requestId));
                    return posAsync.when(
                      ...
                      data: (pos) => Card(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              Text('يوجد قبلك', style: Theme.of(context).textTheme.bodyMedium),
                              Text('${pos.peopleAhead} عملاء',
                    ...
                    child: imagesAsync.when(
                      ...
                      data: (images) => ListView(
                        ...
                        children: [
                          ...
                          if (isActive)
                            InkWell(
                              onTap: () => _addImage(context, ref, req.customerId),
                              child: Container(
                                width: 100,
                                height: 100,
